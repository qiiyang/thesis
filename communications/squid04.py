...
if __name__ == '__main__':
    ...
    #sub_b.errorbar(data["T"], (data["M"]) / M_unit, yerr=NSE * data["err_M"] / M_unit, color="C0", marker="x", markersize=markersize, markerfacecolor="none", linestyle="None", linewidth=1, zorder=1)
    
    """
    data = data.mask(data["T"]> 16.5)
    data = data.mask(data["T"]< 22)
    func = get_func(data)
    params0 = np.array([3.e-5, 15.5, 2.e-6])
    params = opt.leastsq(func, params0, maxfev=1000)[0]
    print(params)
    
    C, Tc, M0 = params
    
    xs = np.arange(16, 25, 0.1)
    ys = C / (xs - Tc) + M0
    sub_b.plot(xs, ys / M_unit, color="k", marker=None)
    """
    #plt.legend()
